...
> 公開・録画前の操作順です。ローカル実素材E2Eは完了していますが、推奨clipの
> ユーザー確認は未完了です。実メディア区間は、人による映像確認と公開用sanitizationが
> 完了するまで撮影済みと扱いません。実動画のクラウド送信は現在も未承認です。
...
**REAL MEDIA GATE**。14 MP4を使うローカルE2E自体は成功しているが、v4aの自動
候補品質は`PARTIAL`で、映像証拠`confirmed`は0。ユーザーが採用したclipだけを
公開用に別途sanitizationし、ファイル名・path・座標・時刻を見せない画面が準備
できた場合にだけ収録する。それまでは合成プレースホルダーを実出力と誤認させず、
この区間を後日差し替える。
