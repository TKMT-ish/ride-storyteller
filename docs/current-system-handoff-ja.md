...
## 196. 7.6 署名付きURL——`app/signed_links.py`

第195節が「次に推奨」と書いた一片。`app.tenancy` が**どの object に届くか**を、
`app.retention` が**いつまで在るか**を、`app.tenant_migration` が**口座より前からある
object をその二つの規則の中へ**入れた。まだ無いのは、利用者が保存された縮小コピーを
再び見る唯一の道——URL である。

署名付きURLは **bearer capability**（持っている者がその利用者になる、期限まで、追加の
検査なしに）。だから問いは全て**署名が存在する前に**答えねばならず、使われる時ではない。
規則は3つ、前の3 module と同じく意図的に鈍い:

- **所有の検査は署名の前**。`require_own_object` を最初に、判定・順位付けの前と同じ
  「出て行く時」に呼ぶ。package に保存された URI・backup から戻した URI・手で編集された
  URI は所有の証拠ではなく、検査せずに署名を被せることは、その**非・証拠をアクセスに
  変えてしまう**。
- **リンクは指す object より長生きしない**。寿命は「自分の上限」と「保持期間の残り」の
  短い方。期限を過ぎても解決するリンクは破られた約束で、解決しなくなるリンクは
  「守られた約束が不具合として現れたもの」。両方とも、署名時に
  `app.retention.deletion_deadline` を訊けば避けられる。上限そのものは日ではなく分
  （既定300秒・上限900秒）——URLは一度発行されると chat log・履歴・screenshot に複製され、
  そこでは「これは capability である」ことが目に見えないから。
- **読みだけ**。書ける署名付きURLは別の約束（他人の ride が**見られる**のではなく
  **置き換えられる**）なので、既定を持つのではなく **method 引数を取らない**。

拒否文は object を復唱しない（`app.tenancy` と同じ）。**発行できたリンクは object 名を
必ず含む**——それが URL というものだから、返り値から隠すのは芝居である。**CLI は作らない**:
`migrate`・`sweep` は人が承認するから CLI が要ったが、リンクは既に利用者の代理で訊いて
いる相手へ発行するものであり、出力が動く capability である端末コマンドは、上の
「log に複製される」危険が prompt を着ただけになる。

**実測（実素材2 ride、941窓＝proxy ファイル数、外部送信なし・支出¥0。GCS client は
差し込んだ模造で、1バイトも送っていない）**:

| 走行が upload された時点 | bridge-e2e-v1（187窓） | day-2-v1（754窓） |
|---|---|---|
| 直後 | 187/187 発行・寿命 300秒 | 754/754 発行・寿命 300秒 |
| 29日前 | 187/187・300秒 | 754/754・300秒 |
| 期限の10分前 | 187/187・300秒 | 754/754・300秒 |
| 期限の4分前 | 187/187・**240秒に短縮** | 754/754・**240秒に短縮** |
| 期限ちょうど | **拒否**（署名呼び出し0回） | **拒否**（署名呼び出し0回） |
| 期限の1日後 | **拒否**（署名呼び出し0回） | **拒否**（署名呼び出し0回） |

**object の期限を越えて有効なリンクは全941窓で0本**。他人（別口座）から届くリンクは
**0/941**で、しかも**署名呼び出しが1回も発生しない**（検査が署名の前にある、の実測）。

**テスト**: 全**2,978件成功**（`--deselect tests/test_judged_film_end_to_end.py` の6件のみ
既知事由で除外。新規36件——`tests/test_signed_links.py`）。Ruff check 緑、`ruff format`
済み、`git diff --check`（自分の変更ファイルのみ）問題なし。

**触れなかったもの**: `app/web/private_journey_console.py`・`app/web/server.py`
（未commitの変更あり）、`app/data_handling_disclosure.py`・
`tests/test_data_handling_disclosure.py`（未追跡、7.6「同意表示」の別層作業と
見られる）——この節の開始時点で既に作業ツリーにあり、自分が変更したものではないため
規約どおり触っていない（第192・194・195節から継続、依然未commitのまま）。`dev` の
`cloud/*` branch は無かった（`git fetch dev` で確認）。全行程バッチの最終行は 2026-09-06
12:00 で、走っていない。

**E-5 を選ばなかった理由**（優先順では 7.x より上）: E-5 の配線は
`app/story_film.py` が全窓の音声を落としている（`-an`）ところを開けることで、
「素材の音を聞かせるか」自体がオーナーに向いた判断であり、かつ roadmap が
「別層が ambient を配線中の間は避ける」と書いている。その別層の `app/story_ambient.py`
は依然 commit も未追跡ファイルも無く（`story_film.py:212` の comment だけが参照している）、
着手の可否をこの層から判断できないため見送った。**E-5・E-7 の配線はこの一点で止まって
いる**——ambient 層が着地しないなら、どこかの層が `story_film.py` の音声経路ごと引き取る
判断が要る（オーナー判断ではなく設計判断）。

**次に推奨**: 7.6 の残りは「認証そのもの」「同意表示（別層が着手中）」「バケット側
ライフサイクル規則との二重化」。**二重化**が次の素直な一片——`app.retention` の定数から
...
### 追記（2026-09-09 23:15）: 提出完了

オーナーがデモ動画を YouTube に公開（<https://youtu.be/wgrmPRAei-o>、公開・2:57・英語字幕・説明文に音楽クレジット、
外から確認済み）し、Devpost に **Submit 済み**。締切 06:00 JST までは編集可能。残る任意の
手直しは、審査員 PDF の差し替え（新 SHA・動作環境）と画像 5 の差し替えのみ。

## 214. ハッカソン向け開発の完了（2026-09-09 23:40 JST、オーナー宣言）

Agentic Cinema への提出は 2026-09-09 23:00 頃（JST）に完了し、オーナーは「ハッカソン向けの開発は
